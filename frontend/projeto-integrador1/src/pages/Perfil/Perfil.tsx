...
const Perfil: React.FC = () => {
  ...
  useEffect(() => {
    if (novoEmail || novaSenha || novoContato) {
      setHasUnsavedChanges(true);
    } else {
      setHasUnsavedChanges(false);
    }
  }, [novoEmail, novaSenha, novoContato]);
  ...
  const handleUpdate = (e: React.FormEvent) => {
    ...
    setUsuario(prevUsuario => ({
      ...prevUsuario,
      email: novoEmail || prevUsuario.email,
      contato: novoContato || prevUsuario.contato,
      // A senha não é exibida, então não a incluímos no estado 'usuario'
    }));
  ...
  const handleDeleteConfirm = () => {
    ...
    // Simulação de exclusão: redireciona para a tela inicial
    navigate('/');
  };
